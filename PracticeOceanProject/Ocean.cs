...
namespace PracticeOceanProject
{
    internal class Ocean
    {
        ...
        public void Initialize(Viewer myViewer)
        {
            ...
            myViewer.InputInfo(ref numPrey, ref numPredators, ref numObstacles, ref numIterations);
            InitOcean();
            PlaceObjects();
        }
        ...
        {
            Console.WriteLine(DisplayStatistics());
            Console.WriteLine(DisplayBorder());
            for (int row = 0; row < numRows; row++)
            {
                for (int col = 0; col < numCols; col++)
                {
                    Console.Write(cells[row, col].Show());
                }
                Console.WriteLine();
            }
            Console.WriteLine(DisplayBorder());
        ...
        {
            string stats;
            int preyQuantity = CountPrey();
            int predQuantity = CountPredators(); 

            stats = String.Format("Iteration: {0}\nNumber of prey: {1}\nNumber of predators: {2}", iterationCounter, preyQuantity, predQuantity);
            return stats;
        }
